...
void ProblemMap::Problem59()
{
	std::ifstream myfile("Resources/p059_cipher.txt");
	...
	string_vector stringTokens = Utils::split(data, ',');

	char_vector tokens;
	
	std::transform(stringTokens.begin(), stringTokens.end(), std::back_inserter(tokens), [](const std::string& str) { return stoi(str); });

	std::string result;

	int_vector tokenCounts[3] = { int_vector(127), int_vector(127), int_vector(127) };

	for (int i = 0; i < tokens.size(); ++i)
	{
		int j = i % 3;
		tokenCounts[j][tokens[i]]++;
		if (tokenCounts[j][tokens[i]] > tokenCounts[j][key[j]])
			key[j] = tokens[i];
	}
	...
	for (int i = 0; i < tokens.size(); ++i)
		result.push_back(tokens[i] ^ key[ i % 3]);
